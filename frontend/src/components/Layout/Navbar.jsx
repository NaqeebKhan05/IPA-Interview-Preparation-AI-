import React from "react";
import { Link } from "react-router-dom";
import ProfileInfoCard from "../Card/ProfileInfoCard";
import LOGO_IMG from "../../assets/logo.png";

const Navbar = () => {
  return (
    <div className="h-16 bg-white border-b border-gray-200/50 backdrop-blur-[2px] py-2.5 px-4 md:px-0 sticky top-0 z-30 ">
      <div className="container ms-auto flex items-center justify-between gap-5">
        <Link to="/dashboard">
          <h2 className="flex items-center gap-2 text-lg md:text-xl font-medium text-black leading-5">
            <img
              src={LOGO_IMG}
              alt="Interview Prep AI Logo"
              className="h-10 w-auto object-contain rounded-full"
            />
            Interview Prep AI
          </h2>
        </Link>

        <ProfileInfoCard />
      </div>
    </div>
  );
};

export default Navbar;
